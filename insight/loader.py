...
class Loader(Transfer):
    ...
    def load(self, load_config: dict):
        ...
        client_set = self.subscription_dict.get((src_topic_id, src_table_id), [])
        if load_config['client_id'] in client_set:
            dispatcher = self.client_dict.get(load_config['client_id'], None)
        if not dispatcher:
            logging.warning("{}-{}: Source Table / Topic is not subscribed".format(src_topic_id, src_table_id))
            return
        # Check if target exists
        target_list = [(i['topic_id'], i['table_id']) for i in dispatcher.get_destinations(src_topic_id, src_table_id)]
        if (tar_topic_id, tar_table_id) not in target_list:
            logging.warning("{}-{}: Target Table / Topic is not subscribed".format(tar_topic_id, tar_table_id))
            return
        # Loader Setting
        ...
        self.archiver.set_current_topic_table(src_topic_id, src_table_id)
        # Initial Loading
        if 'init_load' in load_config and load_config['init_load']:
            doc_ref = None
            header_ref = self.depositor.get_table_header()
            header_dict = self.depositor.get_dict_from_ref(header_ref)
            for doc_ref in self.depositor.get_stream_by_sort_key(['packaged', 'merged', 'initial']):
                break
            if not doc_ref:
                logging.warning("{}-{}: No Data to load".format(tar_topic_id, tar_table_id))
                return
            doc_dict = self.depositor.get_dict_from_ref(doc_ref)
            new_load_config = load_config.copy()
            new_load_config.pop('head_load')
            new_load_config['start_key'] = get_sort_key_from_dict(doc_dict)
            for doc_ref in self.depositor.get_stream_by_sort_key(['merged', 'initial']):
                break
            # Case 1 : No merged or initial data to load, launch package load process only
            if not doc_ref:
                for doc_ref in self.depositor.get_stream_by_sort_key(['packaged'], reverse=True):
                    break
                doc_dict = self.depositor.get_dict_from_ref(doc_ref)
                new_load_config['end_key'] = get_sort_key_from_dict(doc_dict)
                self.messager.trigger_load(new_load_config)
            # Case 2 : Launch package load process and continue
            else:
                # Step 2.1 Check if the package load process should run
                package_ref = None
                for package_ref in self.depositor.get_stream_by_sort_key(['packaged'], doc_ref, reverse=True):
                    break
                if package_ref:
                    package_dict = self.depositor.get_dict_from_ref(package_ref)
                    new_load_config['end_key'] = get_sort_key_from_dict(package_dict)
                    self.messager.trigger_load(new_load_config)
                # Step 2.2 Dispatch Header data
                tar_header_data = header_dict
                tar_header_body = tar_header_data.pop('data')
                dispatcher.dispatch(tar_header_data, tar_header_body, None, tar_topic_id, tar_table_id)
                # Step 2.3 Start Merge Process
                for doc_ref in self.depositor.get_stream_by_sort_key(['packaged', 'merged', 'initial']):
                    doc_dict = self.depositor.get_dict_from_ref(doc_ref)
                    # Step 2.3.1 Package Type -> Start a new job
                    if doc_dict['merge_status'] == 'packaged':
                        new_sorted_key = get_sort_key_from_dict(doc_dict)
                        new_load_config.update({'start_key': new_sorted_key, 'end_key': new_sorted_key})
                        self.messager.trigger_load(new_load_config)
                    # Step 2.3.2 Other Type
                    else:
                        tar_header_data = doc_dict
                        tar_header_body = json.loads(
                            encoder(tar_header_data.pop('data'), self.depositor.data_encode, 'flat'))
                        dispatcher.dispatch(tar_header_data, tar_header_body, None, tar_topic_id, tar_table_id)
        # Package Loading
        elif 'start_key' in load_config and 'end_key' in load_config:
            start_key, end_key = int(load_config['start_key']), int(load_config['end_key'])
            mid_key = str(( start_key + end_key ) // 2)
            # Step 1: If start == end, do the job
            if start_key == end_key:
                for doc_ref in self.depositor.get_stream_by_sort_key(['packaged'], mid_key):
                    tar_header = self.depositor.get_dict_from_ref(doc_ref)
                    self.archiver.load_archive(tar_header['merge_key'])
                    tar_file_data = self.archiver.get_data()
                    dispatcher.dispatch(tar_header, None, tar_file_data, tar_topic_id, tar_table_id)
                    return
            # Step 2: Get the right start key
            for right_doc_ref in self.depositor.get_stream_by_sort_key(['packaged'], mid_key, False, 0, False):
                right_start_key = get_sort_key_from_dict(self.depositor.get_dict_from_ref(right_doc_ref))
        ...
        # Shoudn't happen
            ...
            logging.warning("{}-{}: No load instruction got".format(src_topic_id, src_table_id))
            return
    """